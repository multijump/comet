...
import { isRewardSupported, isBulkerSupported, isInNetwork, getExpectedBaseBalance } from './utils';
import { exp } from '../test/helpers';

scenario(
  'Comet#bulker > all non-reward actions in one txn',
  {
    filter: async (ctx) => !isInNetwork(ctx, ['polygon']) && await isBulkerSupported(ctx),
    tokenBalances: {
...
scenario(
  'Comet#bulker (Polygon) > all non-reward actions in one txn',
  {
    filter: async (ctx) => isInNetwork(ctx, ['polygon']) && await isBulkerSupported(ctx),
    tokenBalances: {
      albert: { $base: '== 0', $asset0: 3000 },
